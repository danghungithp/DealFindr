...
export default function HomePage() {
  const [productSearchTerm, setProductSearchTerm] = useState(""); // Stores the term used for the search
  ...
  const handleSearch = async (searchTerm: string) => {
    ...
    const operations = [
      ...
      },
      {
        ...
        condition: true, // Always attempt, flow might handle URL or name
      },
      {
    ...
    // Execute all operations, some might be skipped based on condition
    await Promise.all(
      operations.map(async (op) => {
        if (op.condition) {
          try {
            const result = await op.fn();
            op.setData(result as any); // Cast as any due to generic setData
          } catch (e: any) {
            console.error(`${op.name} Error:`, e);
            toast({
              variant: "destructive",
              title: `Error fetching ${op.name}`,
              description: e.message || "An unexpected error occurred.",
            });
            op.setData(null);
          }
        } else {
          op.setData(null); // Explicitly set to null if not attempted
        }
      })
    );
    ...
    <div className="container mx-auto p-4 md:p-8 min-h-screen flex flex-col">
      <header className="text-center my-8 md:my-12">
        ...
        <p className="text-muted-foreground mt-3 text-lg md:text-xl">
          Your smart shopping assistant for the best deals and insights.
        </p>
      </header>
      ...
      <main className="flex-grow">
        ...
        {!productSearchTerm && !isLoading && (
          <div className="text-center text-muted-foreground mt-16 animate-fadeIn">
            ...
            <p className="text-2xl font-medium">Discover Product Insights</p>
            <p className="text-lg">Enter a product name or link above to get started!</p>
          </div>
        )}

        {isLoading && !productSearchTerm && ( /* Show spinner if loading before any search term */
          ...
          <div className="mt-10 space-y-6">
            {isLoading && (
                <div className="flex justify-center items-center py-10">
                    <LoadingSpinner size={48} />
                    <p className="ml-4 text-lg text-muted-foreground">Fetching product data...</p>
                </div>
            )}
      ...
      <footer className="text-center py-8 mt-12 border-t">
        <p className="text-sm text-muted-foreground">
          &copy; {new Date().getFullYear()} DealFindr. All rights reserved.
        </p>
      </footer>
    </div>
  );
